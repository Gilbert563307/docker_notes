...
class Helpers {
    ...
    /**
     * 
     * @param {*} useSearchParamsUtil 
     * @param {*} useLocationUtil 
     */
    constructor(useSearchParamsUtil, useLocationUtil){
        this.#useSearchParamsUtil = useSearchParamsUtil;
        this.#useLocationUtil = useLocationUtil;
    }


   getItemsPerPage() {
    const itemsPerPage = this.getUrlParams(ITEMS_PER_PAGE);
   ...
   */
   convertToDutchDateFormat(created_at) {
   ...
   * @returns
   */
   formatToDutchStringDate(dateString) {
   ...
   */
   checkForInvalidFields(errors, index, fieldName) {
   ...
   */
   getCurrentPathName() {
    const pathName = this.#useLocationUtil.pathname;
    if (!pathName) return "";
   ...
   */
   getUrlParams(paramName) {
    const [searchParams, setSearchParams] = this.#useSearchParamsUtil;
   ...
   */
   isTheCurrentFilterActive(filterToCheck) {
    const currentFilterToCheckName = filterToCheck;
    const allFilters = this.getAllSessionFilters();
   ...
   */
   setCustomSearchParamToSessionMemory(customSearchParam, valueToSet) {
    const [searchParams, setSearchParams] = this.#useSearchParamsUtil;
   ...
   */
   getTheCurrentItemsPerPage() {
    return parseInt(Helpers..getUrlParams(ITEMS_PER_PAGE) || DEFAULT_ITEMS_PER_PAGE);
  }
...
const helpers = new Helpers(useSearchParams(), useLocation());
