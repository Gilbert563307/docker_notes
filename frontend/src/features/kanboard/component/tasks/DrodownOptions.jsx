...
export default function DrodownOptions({
  statusDropDownRef,
  priorityDropDownRef,
  status,
  setStatus,
  ...
  const statusDropDownItems = [
    {
      content: TASKS_STATUS.BACKLOG,
      onclick: () => toggleSelectedStatusDropDownItem(TASKS_STATUS.BACKLOG),
    },
    {
      content: TASKS_STATUS.TODO,
      onclick: () => toggleSelectedStatusDropDownItem(TASKS_STATUS.TODO),
    },
    {
      content: TASKS_STATUS.IN_PROGRESS,
      onclick: () => toggleSelectedStatusDropDownItem(TASKS_STATUS.IN_PROGRESS),
    },
    {
      content: TASKS_STATUS.COMPLETED,
      onclick: () => toggleSelectedStatusDropDownItem(TASKS_STATUS.COMPLETED),
    },
  ];

  const priorityDropDownItems = [
    {
      content: TASKS_PRIORITY.LOW,
      onclick: () => toggleSelectedPriorityDropDownItem(TASKS_PRIORITY.LOW),
    },
    {
      content: TASKS_PRIORITY.MEDIUM,
      onclick: () => toggleSelectedPriorityDropDownItem(TASKS_PRIORITY.MEDIUM),
    },
    {
      content: TASKS_PRIORITY.HIGH,
      onclick: () => toggleSelectedPriorityDropDownItem(TASKS_PRIORITY.HIGH),
    },
  ];
  ...
  return (
    <React.Fragment>
      ...
      <div className="col-md-2 mb-2">
        ...
        <div className="dropdown">
          <button
            className={`btn-status-dropdown dropdown-toggle ${priorityParentBtnStatus} `}
            type="button"
            data-bs-toggle="dropdown"
            aria-expanded="false"
              ...
              return (
                <li key={index}>
                  <a
                    className="dropdown-item"
                    href="#"
                    onClick={priorityItem.onclick}
                  >
                    {getPriorityBadge(priorityItem.content)}
                  </a>
                </li>
              );
            })}
